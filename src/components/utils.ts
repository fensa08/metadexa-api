...
export default async function simulateTransaction(
	...
	try {
		...
		const afterEthBalance = isNonNativePaymentToken
			? resultData[5]?.returnData
			: resultData[2]?.returnData;

		const paymentFeeBalance = isNonNativePaymentToken
			? resultData[2]?.returnData
			: paymentFees;

		const paymentFeeBalanceBN = web3.utils.toBN(paymentFeeBalance);
		const ethBalanceDiff = web3.utils
			.toBN(afterEthBalance)
			.sub(web3.utils.toBN(beforeEthBalance));
